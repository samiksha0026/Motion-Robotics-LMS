...
export default function StudentLabPage() {
  ...
  async function fetchLab() {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/student/lab`, {
        credentials: "include",
        headers: {
      ...
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-semibold text-gray-900 dark:text-white">Our Lab</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm mt-1">Your school is setting up the lab gallery.</p>
          </div>
        ) : (
          <>
            {/* School name */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {labInfo.labDescription && (
                  <div className="sm:col-span-3 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                    <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">About Our Lab</p>
            ...
            <div>
              <h3 className="font-semibold text-gray-800 dark:text-white mb-3 flex items-center gap-2">
                <Camera className="w-5 h-5 text-indigo-500" /> Lab Photos
              ...
              {labInfo.photos.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {labInfo.photos.map((photo, idx) => (
                    <button key={photo.id} onClick={() => openLightbox(idx)}
                      className="relative aspect-square rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-700 hover:ring-2 hover:ring-indigo-400 transition-all group">
